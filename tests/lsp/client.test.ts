...
describe("LspClient", () => {
	...
	test("initializes and shuts down cleanly", async () => {
	...
	test("documentSymbol returns symbols for a file", async () => {
		client = await createLspClient({
			command: "typescript-language-server",
			args: ["--stdio"],
			rootUri: `file://${FIXTURE_DIR}`,
	...
	test("outgoingCalls returns calls from a function", async () => {
		client = await createLspClient({
			command: "typescript-language-server",
			args: ["--stdio"],
			rootUri: `file://${FIXTURE_DIR}`,
