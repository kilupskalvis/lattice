...
describe("buildGraph", () => {
	test(
		"builds graph with cross-file edges and tags",
		async () => {
			...
			const edge = db
				.query(
					"SELECT * FROM edges WHERE source_id LIKE '%processOrder' AND target_id LIKE '%save'",
				)
				.get();
	...
	test(
		"returns zero stats for empty project",
		async () => {
			const db = createDatabase(":memory:");
			const result = await buildGraph({
